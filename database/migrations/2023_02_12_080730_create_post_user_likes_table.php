...
class CreatePostUserLikesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('post_user_likes', function (Blueprint $table) {
            ...
            $table->foreign('post_id', 'pul_post_fk')->on('posts')->references('id')->onDelete('cascade')->onUpdate('cascade');
            $table->foreign('user_id', 'pul_user_fk')->on('users')->references('id')->onDelete('cascade')->onUpdate('cascade');
        });
    }
